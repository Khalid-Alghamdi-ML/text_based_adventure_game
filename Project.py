...
def main_room():
    ...
    while True:
        player_choice = option("choice")
        # indicating that the first option already choosed and the item already taken
        if player_choice == "1":
...
def white():
    msg("\nWhen you open the door you noticed a shiny object at the right corner of the room\n")
    ...
    if gun == False: 
        while True:
            ...
            if player_choice == "1":
                ...
                print("\nYou run to the first room and close the dore behind you\n")
                main_room()
                break
    elif gun == True:
        while True:
            player_choice = option("choice")
            if player_choice == "1":
                msg("\nYou use the gun to shoot the " + rand() + " and manage to kill it")
